...
import { Weapon } from '../miao-plugin/models/index.js'
import { Data } from '../miao-plugin/components/index.js'
const _path = process.cwd()

const NEWLINE = (os.platform() === 'win32') ? '\r' : '\n'

export class ImportGcLog extends plugin {
  constructor () {
    super({
      name: '提瓦特小助手抽卡记录',
  ...
  async getGcLog (e) {
    ...
    if (ret) {
      ...
      if (!authkey) {
        this.e.reply('请私聊发送抽卡记录链接', false)
    ...
    if (authkey) {
      this.e.reply('抽卡记录链接校验成功，开始获取【提瓦特小助手】小程序抽卡记录，请稍后。。。', false, { at: true })
    }
  ...
  async _getGcLog (e) {
    ...
    let list = json?.list
    if (!json || !list) {
      await this.e.reply(`更新失败 ${json?.result}`, false, { at: true })
      logger.mark(json)
      return true
    }
    if (!list?.length || list?.length <= 0) {
      await this.e.reply('"提瓦特小助手"抽卡记录为空', false, { at: true })
      return true
    }
    let data = await dealJson(list, e)
    if (!data) return true
    /** 保存json */
    let msg = `uid:${this.e.uid}\n更新小助手抽卡记录成功...\n`
    ...
    for (let type in data) {
      ...
      msg += `\n${this.typeName[type]}记录：${data[type].length}条，导入${finalJson.length - log.list.length}条，现共${finalJson.length}条`
    }
...
async function dealJson (list, e) {
  let data = {}
  ...
  let fiveRoleList = gsCfg.getdefSet('role', 'other').five
  let fourRoleList = gsCfg.getdefSet('role', 'other').four
  for (let val of list) {
    if (!data[val.uigf_gacha_type]) data[val.uigf_gacha_type] = []
    if (!val.rank_type) {
      if (val.item_type === '角色') {
        let roleId = gsCfg.roleNameToID(val.name)
        // 判断是否是5星
        if (roleId) {
          roleId = Number(roleId)
          if (fiveRoleList.includes(roleId)) {
            val.rank_type = 5
          } else if (fourRoleList.includes(roleId)) {
            val.rank_type = 4
          } else {
            val.rank_type = 3
          }
        }
      } else if (val.item_type === '武器') {
        let weapon = await Weapon.get(val.name, 'gs')
        const weapons = Data.getData(weapon, 'id,name,star')
        if (weapons && weapons.star) {
          val.rank_type = weapons.star
        }
      }
    }

    data[val.uigf_gacha_type].push(val)
  }
...
async function getUid (e) {
  const uidKey = `Yz:genshin:mys:qq-uid:${this.e.userId}`
  const uid = await redis.get(uidKey)
  if (uid) {
    return uid
  }
}
